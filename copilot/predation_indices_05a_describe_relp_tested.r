...
For each year \(t\), it builds two mechanistic predator indices:
  ...
  \[
    p_{\text{salmon},t}
    =
      \frac{N_{\text{salmon},t}^m}
    {N_{\text{salmon},t}^m + \left(\alpha F_t e^{-\beta W_t}\right)^m}
    \]

\[
  I_{\text{SSL},t} = \text{SSL}_t \cdot p_{\text{salmon},t} \cdot C_t
  \]
...
---
  ...
  \[
    M_t = Q10^{(T_t - T_{ref})/10}
    \]
\[
  I_{\text{Shark},t} = \text{Shark}_t \cdot M_t \cdot O_t
  \]
...
# 
# Final fitted model x16_sar_t = b0 + b1x07_dfa_cpue_int_spr_jun_hw_t + b2I_SSL_t + b3*I_Shark_t + error_t