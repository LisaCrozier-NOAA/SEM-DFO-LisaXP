...
#update Doug's DFAs and list of which indicators are in which DFAs
      ...
      rankedIndicators[1:2,"rankedIndicators"]

#get altprey assignments
      nccpred <- read.csv(file.path(outputDir, "nccpred_altprey_assigned.csv"), row.names = NULL) %>%
          select(short_name_lower,altprey1,altprey2) 
  ...
  mutate(
    ...
    dfa_cols = case_when(
      ...
      dfa_cols == "X10.PredMammalNCC_DFA1" & grepl("allsealionsbonn", short_name_lower, ignore.case = TRUE) ~ 
        "X10.PredMammalNCC_DFA1_cslbonn",
    ...
    is_2yrlead = str_detect(short_name_lower, regex("_2yrlead", ignore_case = TRUE))
  )

nccpred_resolved %>% select(-rankedIndicators)
unique(nccpred_resolved$short_name_lower)
# ---------------------------------------------------------------------------
...
write.csv(nccpred_dfa_unique, file.path(outputDir, "nccpred_dfa_altprey_assigned.csv"), row.names = FALSE)
