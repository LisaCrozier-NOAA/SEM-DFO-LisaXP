...
#RESULTS
Candidate_Metric                               Assigned_Category Max_R
sardine_northern_bmass_jake     sardine_northern_bmass_jake    Cluster 1: Correlated with Hake (|r| > 0.58) 0.970
herring_bmass_jake                       herring_bmass_jake    Cluster 1: Correlated with Hake (|r| > 0.58) 0.938
abund_sardine_doug                       abund_sardine_doug    Cluster 1: Correlated with Hake (|r| > 0.58) 0.924
abund_herring_doug                       abund_herring_doug    Cluster 1: Correlated with Hake (|r| > 0.58) 0.866
hake_age5plus_2025_doug             hake_age5plus_2025_doug    Cluster 1: Correlated with Hake (|r| > 0.58) 0.755
pacificmackerel_gam_2025_doug pacificmackerel_gam_2025_doug    Cluster 1: Correlated with Hake (|r| > 0.58) 0.702
jackmackerel_gam_2025_doug       jackmackerel_gam_2025_doug    Cluster 1: Correlated with Hake (|r| > 0.58) 0.699
hake_cpue_jake                               hake_cpue_jake    Cluster 1: Correlated with Hake (|r| > 0.58) 0.588
hake_fishery_wa_2025_doug         hake_fishery_wa_2025_doug    Cluster 1: Correlated with Hake (|r| > 0.58) 0.586
sardine_gam_2025_doug                 sardine_gam_2025_doug Cluster 2: Correlated with Sardine (|r| > 0.58) 0.890
anchovy_northern_bmass_jake     anchovy_northern_bmass_jake Cluster 2: Correlated with Sardine (|r| > 0.58) 0.854
x05_herring_gam_doug                   x05_herring_gam_doug Cluster 2: Correlated with Sardine (|r| > 0.58) 0.822
sardine_ncc_doug                           sardine_ncc_doug Cluster 2: Correlated with Sardine (|r| > 0.58) 0.805
x09_dfa_hake_age5plus_doug       x09_dfa_hake_age5plus_doug                            FINAL 1: Hake Anchor 1.000
x05_dfa_abund_sardine_doug       x05_dfa_abund_sardine_doug                         FINAL 2: Sardine Anchor 1.000
sitka_herring_e_go_a_doug         sitka_herring_e_go_a_doug                   FINAL 3: Sitka Herring Anchor 1.000
x05_herring_ncc_doug                   x05_herring_ncc_doug                     FINAL 4: NCC Herring Anchor 1.000
x05_anchovy_gam_doug                   x05_anchovy_gam_doug               FINAL 5: Executive Anchovy Choice 1.000
jack_mackerel_bmass_jake           jack_mackerel_bmass_jake         Uncorrelated / Low Correlation (< 0.58) 0.836




# ==============================================================================
# Script: Ocean Forage Feature Selection & Multi-Panel Analysis
# Threshold: |r| > 0.58 | Excludes Age-0 recruits & Age-1 Hake
# Includes: Safe dir check, clean accounting print, 5-col grid & 5-panel overlay plot
# ==============================================================================
...
# Safe Directory Check: Only create if missing
...
# Explicitly exclude non-prey juvenile life stages (Age-0 recruits & Age-1 Hake)
...
# -----------------------------------------------------------------------------
# 2. Define Primary Anchors & Grouping Analysis (|r| > 0.58)
# -----------------------------------------------------------------------------
...
eval_pool <- setdiff(target_cols, final_5)

hake_cluster    <- get_correlated_series(hake_anchor, eval_pool, cor_mat)
sardine_cluster <- get_correlated_series(sardine_anchor, setdiff(eval_pool, hake_cluster), cor_mat)
sitka_cluster   <- get_correlated_series(sitka_anchor, setdiff(eval_pool, c(hake_cluster, sardine_cluster)), cor_mat)
herring_cluster <- get_correlated_series(herring_anchor, setdiff(eval_pool, c(hake_cluster, sardine_cluster, sitka_cluster)), cor_mat)
anchovy_cluster <- get_correlated_series(anchovy_anchor, setdiff(eval_pool, c(hake_cluster, sardine_cluster, sitka_cluster, herring_cluster)), cor_mat)

# -----------------------------------------------------------------------------
# 3. Accounting Table & Corrected Print Call
# -----------------------------------------------------------------------------
...
cat("\n========================================================\n")
cat("       COMPLETE CANDIDATE METRIC ACCOUNTING             \n")
cat("========================================================\n")
# Fixed print call ensuring no 'na.print' error
print(as_tibble(accounting_df), n = Inf)
...
# -----------------------------------------------------------------------------
# 6. Plot 2: New 5-Panel Overlay View (Anchor Bold Dark Line Over Correlated Series)
# -----------------------------------------------------------------------------

# Function to render a single overlay panel
make_overlay_panel <- function(group_id, title_name) {
  ...
  ggplot() +
    # Correlated candidate sub-series in thin grey
    geom_line(
      data = bg_series,
      aes(x = year, y = std_val, group = metric_name),
      color = "grey60", linewidth = 0.7, alpha = 0.7
    ) +
    # Selected Primary Anchor in thick, dark bold line
    geom_line(
      data = anchor_series,
