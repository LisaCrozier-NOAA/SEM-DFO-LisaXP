...
#FOR UC
# beta_sst  beta_herr   beta_cap   alpha_ot 
# 5.0000000 -5.0061780  1.5153762  0.3998031 


# Regressions:
#                 Estimate  Std.Err  z-value  P(>|z|)   Std.lv  Std.all
# sar_uc ~                                                              
# x09_df_hk_g5pl   -0.170    0.048   -3.504    0.000   -0.170   -0.382
# i_ssl            -0.306    0.070   -4.358    0.000   -0.306   -0.575
# i_shark          -0.299    0.086   -3.497    0.000   -0.299   -0.573
...
salmon_dat<-read.csv(file.path("data_Lisa/sem_master_data.csv"))%>% 
  clean_names()  %>% 
  select(year,contains("x07"),contains("sar_uc"),contains("sar_"),contains("x09_dfa_hake"))
names(salmon_dat)
...
pred_index_dat_uc <- pred_index_dat %>%
  filter(!is.na(sar_uc))
cat("Original rows:", nrow(pred_index_dat), "\n")
cat("Filtered rows (valid sar_uc):", nrow(pred_index_dat_uc), "\n")
#18 years


#SEM---------
# fit_model <- sem('
#             sar_uc ~ x09_dfa_hake_age5plus + i_ssl + i_shark
#           ', data = pred_index_dat_uc, std.lv = TRUE, missing = "ML", warn = FALSE)
...
ssl_s   <- safe_scale(pred_index_dat_uc$ssl_seak_pup_pred)
sst_s   <- safe_scale(pred_index_dat_uc$sst_wgoa_coastwatch_junjulaug)
herr_s  <- safe_scale(pred_index_dat_uc$egoa_bio_stka_herr_matbiom)
cap_s   <- safe_scale(pred_index_dat_uc$wgoa_bio_mid_il_capelin)
shark_s <- safe_scale(pred_index_dat_uc$goa_pacific_sleeper_shark)

t_raw   <- pred_index_dat_uc$sst_wgoa_coastwatch_junjulaug
t_ref   <- mean(t_raw, na.rm = TRUE)
t_ot    <- pred_index_dat_uc$pdo_djf

pred_index_dat_uc_fitted <- pred_index_dat_uc %>%
  mutate(
