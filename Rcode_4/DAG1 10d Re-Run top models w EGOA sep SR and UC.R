#Input data----------
              ...
              guild.dfas1<-read.csv("LisaXP/outputs_4/guild.dfa.NCC.AK.csv",row.names=1) %>%
                ...
                inner_join(trend_df_1998_herring_egoa,by="year")
                  ...
                  topmodel1<-read.csv("LisaXP/outputs_4/Doug.topmodels100.csv",row.names=1) %>%
                    group_by(model_id) %>%
                    arrange(AIC) %>%
                    slice(1)
                  topmodel1
...
#DAG1C long  =========================
    ...
    #orig aic: 74.782
          #replaced X12_copepodCom_EGoA with wgoa_cap.pcod #aic up: 88.434 
          #replaced X12_copepodCom_EGoA with egoa_herring #aic up: 84.957 
          #replaced X06_Lmu_IntSprJunH with egoa_herring #aic up: 100.052  
    single_model_text <- glue('
          ...
          # 0.802  1.000 74.782  0.000  0.981 
          # 
