#Prompt for gemini to go from doug's output to paper tables

#Goal 1: variable importance plot and aic table of all models
    #File 1. DAG1 00 Main analysis.R: --------
    ...
    #File 2. DAG1 05 varimportance_plot.r
    # 2. DAG1A and DAG1B -------------
          # inputs: importance_topvar
          #         importance_topvar<-read.csv("LisaXP/outputs_4/importance_topvar_daic3.csv",row.names=1);importance_topvar
          # output:       ggsave( "LisaXP/outputs_4/DAG1A_DAG1B_importance_topvar_barchart.png",importance_topvar_barchart)
 



#Goal2: Hypothesis testing
      #File 1: DAG hypothesis plot -- DONE
                    
      #File 2: DAG1 100 MainRegression_Significance_Summary.csv: 
              #DAG1 100c MainRegression_Significance_Summary.csv: 
              #DAG1 101 Indicator Sign Explanatiohs,r
            inputs: rankings_all (SEMresultsByClus.csv w/o HCI, 
                    estimates_all<-parameterEstimates.csv)
             #output: write_csv(summary_table_no_spurious, "LisaXP/outputs_4/MainRegression_Significance_Summary_NoSpuriousIndicators.csv")
            ...
            # 
# DONE:      
#       That table is fine for the appendix, but I want a prettier way to present synthesized output
#        H1: bottomup regulation: prey->growth (ncc), growth->abund (cpue, sar), akprey->sar (ak)
#        H2: topdown regulation: pred->cpue (ncc), pred ->sar (ak), akprey->sar (ak)
#        H3: indirect effects (alt prey and habitat compression index): pred*prey (ncc, ak), pred*SST (=hci?)
#        
