...
# Goal 2: Evaluate Path Significance, Competitor Direction & Table 3 Generation
# Output: Rcode_for_paper/Routput_for_paper/tables/ and data/
# ==============================================================================
...
# ------------------------------------------------------------------------------
# STEP 0: SETUP PATHS & LOAD CROSSWALK UTILITIES
# ------------------------------------------------------------------------------
proj_dir     <- getwd()
...
# Active Baseline Models (DAG 1A and DAG 1B across long and short windows)
...
cat("Running Goal 2 Path Significance & Competitor Evaluation Pipeline...\n")

# ------------------------------------------------------------------------------
# STEP 1: LOAD MODEL RANKINGS & PURGE HCI + CR SEALS
# ------------------------------------------------------------------------------
cat("Loading model rankings (SEMresultsByClus.csv)...\n")
...
# Unnest generic node indicator columns
...
# Identify and globally purge model runs with CR Harbor Seals
cr_seal_keys <- rankings_long %>%
  filter(LisaName %in% c("X10_Harbor_seal_CR_2yrLead", "X10_Harbor_seal_CR")) %>%
...
# Extract Top 100 clean models per model run
...
# ------------------------------------------------------------------------------
cat("Loading parameter estimates (parameterEstimates.csv)...\n")
...
# ------------------------------------------------------------------------------
# STEP 3: SAVE INDICATOR-SPECIFIC DIAGNOSTIC TABLE
# ------------------------------------------------------------------------------
indicator_diagnostic_summary <- targeted_estimates %>%
  filter(is_top_100 == TRUE) %>%
  group_by(model_id, path_label, H_code, rhs, LisaName, Expected_Sign) %>%
  summarise(
    N_Top100_Runs   = n(),
    Pct_Significant = round((sum(is_significant) / N_Top100_Runs) * 100, 1),
    Pct_Positive    = round((sum(is_positive) / N_Top100_Runs) * 100, 1),
    Pct_Negative    = round((sum(is_negative) / N_Top100_Runs) * 100, 1),
    Pct_Supported   = round((sum(Model_Supports) / N_Top100_Runs) * 100, 1),
    Mean_Beta       = round(mean(est, na.rm = TRUE), 3),
    .groups         = "drop"
  ) %>%
  arrange(H_code, path_label, model_id, desc(N_Top100_Runs))

write_csv(indicator_diagnostic_summary, file.path(data_out_dir, "Goal2_Indicator_Specific_Diagnostics.csv"))

# ------------------------------------------------------------------------------
# STEP 4: GENERATE TABLE 3 (WIDE FORMAT HYPOTHESIS SUMMARY)
# ------------------------------------------------------------------------------
path_summary_by_model <- targeted_estimates %>%
  ...
  summarise(
    ...
    N_Competitors     = sum(is_competitor),
    .groups           = "drop"
  ) %>%
  mutate(
...
table3_gt <- table3_wide %>%
  ...
  tab_header(
    ...
    subtitle = "Path significance (% Sig at p < 0.05) and directional support across top-ranked candidate models (accounting for competitor species signs)"
  ) %>%
  cols_label(
    H_code               = md("**Code**"),
...
message("Goal 2 Pipeline Complete! Output files saved to ", master_out)
